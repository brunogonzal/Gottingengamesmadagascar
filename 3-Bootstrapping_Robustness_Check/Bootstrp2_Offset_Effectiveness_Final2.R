...
FE_bootstrap_results <- FE_bootstrap_results |>
  add_row(Tr = FE_all$coefficients[1],#save coefficient estimates
          Tr_std_err = tidy(FE_all)$std.error[1]#save standard errors, we resample later from all estimates 
          )
...
FE_bootstrap_results <- FE_bootstrap_results |>
  rowwise() |>
  mutate(Tr_distribution = list(rnorm(1000, mean = Tr, sd = Tr_std_err))) |> # Take 1000 samples from each estimate 
  unnest(Tr_distribution) |> # equals 1000*1000=10^6 point estimates of the true effect
  dplyr::select(Tr_distribution) # kick the other variables
...
# Plot each result 
# 
# ggplot(FE_bootstrap_results, aes(x = Tr_distribution)) +
...
# The distributions looks pretty normal so the end results are 
# 
# 
# 
# mean(FE_bootstrap_results$Tr_distribution)
# sd(FE_bootstrap_results$Tr_distribution)
