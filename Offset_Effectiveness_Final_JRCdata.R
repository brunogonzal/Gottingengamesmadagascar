...
ANK <- JRC_data[JRC_data$SOURCE == "ANK",]
ANK <- ANK[,c(12,13,14,9,16,4,3,2,10,8,7,6,5,21)]
ANK$treated = 1              
ANK$offset = "ANK"

CFAM <- JRC_data[JRC_data$SOURCE == "CFAM",]
CFAM <- CFAM[,c(12,13,14,9,16,4,3,2,10,8,7,6,5,21)]
CFAM$treated = 1              
CFAM$offset = "CFAM"

CZ <- JRC_data[JRC_data$SOURCE == "CZ",]
CZ <- CZ[,c(12,13,14,9,16,4,3,2,10,8,7,6,5,21)]
CZ$treated = 1              
CZ$offset = "CZ"

Control <- JRC_data[JRC_data$SOURCE == "CONTROL",]
Control <- Control[,c(12,13,14,9,16,4,3,2,10,8,7,6,5,21)]
Control$treated = 0
Control$offset = "Cont"

# Rename columns #

cols <- c("Deforestation", "Degradation", "Pop_density", "Dist_sett", "Slope", "Elevation", "Aspect", "Annual_Rain",
          "Dist_track", "Dist_road", "Dist_river", "Dist_edge", "Dist_defor", "geometry", "treated", "offset")
...
m.data.ANK$Tree_loss <- pmax(m.data.ANK$Deforestation, m.data.ANK$Degradation)
m.data.CZ$Tree_loss <- pmax(m.data.CZ$Deforestation, m.data.CZ$Degradation)
m.data.CFAM$Tree_loss <- pmax(m.data.CFAM$Deforestation, m.data.CFAM$Degradation)
m.data.TTF$Tree_loss <- pmax(m.data.TTF$Deforestation, m.data.TTF$Degradation)

annual_defor_ANK <- data.frame(table(factor(m.data.ANK$Tree_loss, levels = 2001:2023), m.data.ANK$offset))
...
annual_defor_CZ <- data.frame(table(factor(m.data.CZ$Tree_loss, levels = 2001:2023), m.data.CZ$offset))
...
annual_defor_CFAM <- data.frame(table(factor(m.data.CFAM$Tree_loss, levels = 2001:2023), m.data.CFAM$offset))

annual_defor_TTF <- data.frame(table(factor(m.data.TTF$Tree_loss, levels = 2001:2023), m.data.TTF$offset))
...
annual_defor_ANK <- Data_construction_DiD(annual_defor_ANK, 2011)
annual_defor_CFAM <- Data_construction_DiD(annual_defor_CFAM, 2013)
annual_defor_CZ <- Data_construction_DiD(annual_defor_CZ, 2009)
...
annual_defor_ANK$log_annual_defor_newdata <- log(annual_defor_ANK$Annual_Deforestation + 1)
annual_defor_CZ$log_annual_defor_newdata <- log(annual_defor_CZ$Annual_Deforestation + 1)
annual_defor_CFAM$log_annual_defor_newdata <- log(annual_defor_CFAM$Annual_Deforestation +1)
...
ANK_data_before <- annual_defor_ANK[(annual_defor_ANK$Year <2011),]
CFAM_data_before <- annual_defor_CFAM[(annual_defor_CFAM$Year <2013),]
CZ_data_before <- annual_defor_CZ[(annual_defor_CZ$Year <2009),]
TTF_data_before <- annual_defor_TTF[(annual_defor_TTF$Year <2014),]
...
CFAMa <- lm(log_annual_defor_newdata ~ Year*TreatedF, data= CFAM_data_before)
summary(CFAMa)
...
CZa <- lm(log_annual_defor_newdata ~ Year*TreatedF, data= CZ_data_before)
summary(CZa)


# TTF #

TTFa <- lm(log_annual_defor_newdata ~ Year*TreatedF, data= TTF_data_before)
summary(TTFa)
...
modelANK <- lm(log_annual_defor_newdata ~ TreatedF*TimeF, data= annual_defor_ANK)
summary(modelANK) 
confint(modelANK)
...
modelCZ <- lm(log_annual_defor_newdata ~ TreatedF*TimeF, data= annual_defor_CZ)
summary(modelCZ)
confint(modelCZ)
...
modelTTF <- lm(log_annual_defor_newdata ~ TreatedF*TimeF, data= annual_defor_TTF)
summary(modelTTF)
confint(modelTTF)
